...
public class Embedding {
			...
			if (wordVectorFile.endsWith(".gz")) {
				br = new BufferedReader(
						new InputStreamReader(new GZIPInputStream(
								new FileInputStream(wordVectorFile))));
			} else {
	...
	private void loadWordVectors(BufferedReader br) throws IOException {
		...
		while ((line = br.readLine()) != null) {
			...
			if (word.equals("UNKNOWN") || word.equals("UUUNKKK")
					|| word.equals("UNK") || word.equals("*UNKNOWN*")
					|| word.equals("<unk>")) {
				word = UNKNOWN_WORD;
			}
			...
			if (dimOfWords > embeddingSize) {
				if (!warned) {
					warned = true;
					System.err
							.println("WARNING: Dimensionality of numHid parameter and word vectors do not match, deleting word vector dimensions to fit!");
				}
	...
	public static void main(String[] args) throws Exception {
		...
		// }
		SimpleMatrix v1 = embedding.get("stanford");
		SimpleMatrix v2 = embedding.get("computer");
		SimpleMatrix v3 = embedding.get("computers");
		System.out.println("stanford computer: " + v1.dot(v2));
		System.out.println("stanford computers: " + v1.dot(v3));
		System.out.println("computer computers: " + v2.dot(v3));
