...
public class NdcgMain {
	// Note to self: Must change return type
	public static void main(String[] args) throws IOException {
		if (args.length < 2) {
			System.out
					.println("Please specify two files: (i) the ranked input file and (ii) the input file containing the "
							+ "relevance scores.");
			System.exit(1);
		}
		...
		while ((strLine = br.readLine()) != null) {
			if (strLine.trim().charAt(0) == 'q') {
				...
				String[] tokens = strLine.substring(strLine.indexOf(":") + 1)
						.trim().split(" ");
		...
		while ((strLine = br.readLine()) != null) {
			if (strLine.trim().charAt(0) == 'q') {
				...
				if (relevScores.containsKey(query.hashCode() + url.hashCode())) {
					double relevance = relevScores.get(query.hashCode()
							+ url.hashCode());
					rels.add(relevance);
				} else {
					System.err
							.printf("Warning. Cannot find query %s with url %s in %s. Ignoring this line.\n",
									query, url, args[1]);
				}
			}
		}
	...
	private static double getNdcgQuery(ArrayList<Double> rels, double totalSum) {
		...
		for (int i = 0; i < rels.size(); i++)
			localSum += (Math.pow(2, rels.get(i)) - 1)
					/ (Math.log(1 + i + 1) / Math.log(2));
		Collections.sort(rels, Collections.reverseOrder());
		for (int i = 0; i < rels.size(); i++)
			sortedSum += (Math.pow(2, rels.get(i)) - 1)
					/ (Math.log(1 + i + 1) / Math.log(2));
		if (sortedSum == 0)
			totalSum += 1;
		else
